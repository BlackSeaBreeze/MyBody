...
- какие файлы объединены в этот приём (если >1 фото)
- оценка порций (граммы/объём, метод оценки)
- portion_confidence: high/medium/low — насколько уверена оценка веса
...
СВЯЗЬ МЕЖДУ РАЗНЫМИ ФОТО (критично — частая ошибка двойного счёта):
- Разные фото за день — это НЕ автоматически разные съеденные порции. Пользователь фотографирует процесс,
  а не каждый новый приём: упаковка и готовое блюдо, крупный план и общий вид, ингредиенты на доске и то же
  блюдо на тарелке, остатки и «до/после».
- Перед meals_breakdown просмотри ВСЕ фото и сопоставь по времени (timestamp в имени файла), продуктам
  и контексту. Найди дубликаты и «осколки» одного приёма — учти их один раз.
- Типичные ловушки: овощ/ингредиент уже внутри сэндвича/салата и отдельно на следующем фото;
  два ракурса одного сэндвича; упаковка продукта и та же порция в тарелке/стакане.
- Если уверен, что это один продукт/приём — включи в БЖУ один раз; в meals_breakdown укажи,
  какие файлы объединены (например: «фото 7+8 — один сэндвич»).
- Если неясно, дубликат ли это — не дублируй; выбери одну оценку порции и пометь confidence: low/medium.
...
- Щедрые допущения (двойной сыр/ветчина, полная столовая ложка майонеза, 150 г пирога, лишнее масло)
  — только если это явно видно по толщине слоёв, размеру куска относительно посуды, упаковке.
- Скрытое масло/соус, которого не видно — диапазон или минимальная разумная оценка, не верхняя граница.
- При сомнении между лёгкой и тяжёлой версией — бери среднюю или более скромную оценку и поясни в meals_breakdown.
- В каждом приёме укажи `portion_confidence: high/medium/low` и кратко — на чём основана оценка веса.
...
def analyze_food_photos(
    ...
    *,
    ...
    intro = (
        f"Дата: {day_label}.\n\n"
        f"=== Манифест фото ===\n{manifest}\n\n"
        f"Ниже {len(photos)} изображений. Сначала просмотри ВСЕ фото и отметь дубликаты "
        "(один приём/продукт с разных ракурсов — не считай дважды). "
        "Затем meals_breakdown с БЖУ строкой, arithmetic_check (таблица + СУММА), daily_totals = СУММА. "
        ...
        "На фото тарелок и блюд — визуально оценивай состав и объём (полноценные изображения, не OCR). "
